...
- **不要為了餵這個工具去批次抓病歷。** 醫院依法必須記錄每一次存取與複製
  （醫療機構電子病歷製作及管理辦法第 13 條），超出照護關係取得紀錄可能觸及
  刑法第 359 條。同樣的按鍵、不同的範圍：照護自己的病人是有授權的，
  超出照護必要去累積就不是——帳號是你自己的也一樣。
- **去識別化不是往外送的免死金牌。** 憲法法庭 111 年憲判字第 13 號（健保資料庫案）
  認可去識別化的前提是**搭配**目的限制與監督機制。衛福部「醫療機構應用生成式
  人工智慧指引」（衛部醫字第 1151663164 號，2026-05）把在臨床流程使用真實病人資料
  視為「導入」，應走機構評估流程，不算個人測試。合理的階梯：本機模型優先、
  院內部署次之、外部商業模型放最後——而且要先確認自己醫院的資訊使用規範怎麼寫。
